...
RUN apt-get update && apt-get -y install python-rosdep python-rosinstall-generator python-wstool python-rosinstall build-essential cmake wget unzip vim-nox git tmux avahi-utils libnss-mdns dropbear
...
RUN rosinstall_generator ros_comm mobile rosbridge_suite joystick_drivers --rosdistro kinetic --deps --wet-only --tar > kinetic-ros_custom-wet.rosinstall
